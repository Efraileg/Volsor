...
int selector::leerTeclaEspecial(){
    ...
    if(kbhit()){
        ...
        if(car == 27){
            if(kbhit()){
                ...
                if(car == 79){
                    car = getch();
                    funcion = car-79;       //[80,81,82,83] - 79 = [1,2,3,4]
                }
            }   
            else{
...
unsigned char* selector::construirTramaControl(unsigned char direccion, unsigned char control, unsigned char numero){
        return new unsigned char[3]{direccion, control, numero};
    }

unsigned char* selector::construirTramaDatos(unsigned char* tramaControl, int Long, unsigned char* datos, unsigned char BCE){
        int tam = 5+Long;
        unsigned char* tramaDatos = new unsigned char[tam];

        for(int i = 0; i<3; i++){
            tramaDatos[i] = tramaControl[i];
        }
        delete tramaControl;
        tramaDatos[3] = (unsigned char)Long;
...
void selector::mostrarTramas(char* payload, int size)
    {
...
void selector::ComunicacionProtocolo(unsigned char tipoComunicacion, unsigned char *mac_destino){
    ...
    bool emisorOreceptor = false, mismaTrama = false;
    ...
    unsigned char *tramaEnviar = nullptr, *tramaControl = nullptr;
    unsigned char numTrama = '0', BCE = 0, devolverControl = 0;
    int Error = 0, ErrorAcum = 0;

    bool ultimaTrama = false, repetirTrama = true, recibidoACKoNACK = false, insertarErrores = false;
    int contadorTramas = 0, l = 0, longitud = 0;

    if(emisorOreceptor){
        //ES EMISOR -> Deberá cargar del fichero los datos a enviar (en forma de tramas) y constuir la trama de control y la de datos para enviar dicha información.
        
        vector<string> datosGuardados = cargarDatos("EProtoc.txt");     //Se carga en un vector string el fichero de datos.

        while(!ultimaTrama){                                            //Repetir hasta enviar la ultima trama del fichero.
            
            // Cada string del vector de string se copia a un vector "c" de caracteres dinamico.
            // Este vector "c" servirá para calcular el BCE y para construir la trama de datos.

            l = datosGuardados[contadorTramas].size();
            c = new char[l];
            datosGuardados[contadorTramas].copy(c, l);

            contadorTramas++;            // Se emplea un contador para contar cuantos string del vector de string se han leido, es decir, cuantas tramas se han leído.
            ultimaTrama = (contadorTramas == datosGuardados.size());     //if(i ==  datosGuardados.size()) ultimaTrama = true;
            ...
            while(repetirTrama){
                ...
                tramaControl = construirTramaControl(tipoComunicacion, STX, numTrama);
                BCE = calcularBCE((unsigned char*)c, l);
                tramaEnviar = construirTramaDatos(tramaControl, l, (unsigned char*)c, BCE);
                
                // Se lee de teclado.
                insertarErrores = true;
                while(insertarErrores){
                    
                    Error = leerTeclaEspecial();    
                    if(Error == 4){
                        ErrorAcum ++;
                    }

                    // Si se presiona otra tecla que no corresponde con la inserción de errores se deberá limpiar el buffer para evitar confusiones.
                    // Si ya se han leido todos los F4 y se lee algo mas (se ha llegado a leer basura o vacio cualquier tecla en buffer se borrara.

                    else{
                        insertarErrores = false;
                        __fpurge(stdin);        
                    }
                }

                if(ErrorAcum > 0 && !mismaTrama){       // Si hay errores acumulados para enviar y no es la misma trama -> se propaga el error.
                ...
                // Una vez enviada la trama se espera a que el receptor responda con un ACK (lo ha recibido correctamente)
                recibidoACKoNACK = false;
                while(!recibidoACKoNACK){                     // A la espera de que me responda con ACK o bien NACK.
                    aux = recibirTrama(longitud);
                    if(aux != nullptr){
                        if(*(aux+1) == ACK || *(aux + 1) == NACK){
                            recibidoACKoNACK = true;
                            mostrarTramasControl((unsigned char*) aux, 0, 0, 'R');
                            repetirTrama = (*(aux+1) == NACK);    //Se repite la trama si se recibe un NACK o sea, contrario a ACK
                        }
                    }
                }
            }

            numTrama = (numTrama == '0') ? '1' : '0';   //Alternar entre 0 y 1
            delete c;
        }
        ...
        // RECEPTOR -> Deberá recibir la información que el Emisor le envía y comprobar que es correcta (en este caso comparando el BCE).
        ...
        while(!recibirFinal){
            ...
            if(aux != nullptr){
                ...
                if(*(aux+1)==STX) {      // Si es STX se indica que es una trama que recibe o envía datos.
                    
                    // Tras recibir los datos que se deben comprobar que son correctos.
                    // Se calcula el BCE correspondiente al campo de datos que se recibe.
                    // Si dicho calculo coincide con el BCE que se envia en la misma trama, los datos son correctos respondiendo con ACK, de lo contrario, se responde con NACK.
                    ...
                    // Se envia una trama de datos avisando al emisor con ACK o NACK (almacenado en "devolverControl"). 
                ...
                }else if(*(aux+1)==EOT){        // Si es EOT se indica que es el fin de la trama de datos.
                    ...
                    if(estado == 1){            // En el Modo Maestro, esto se encarga de determinar si el maestro acepta cerrar la comunicación.
                        int opc = 0;            // comunicacion en sondeo (T)

                        // Se evita un valor invalido
                        ...
                        // Se continúa con la comunicación.
                        else if (opc == 2){
                    ...
                    // Se envia la respuesta de cierre.
...
void selector::mostrarTramasControl(unsigned char* trama, unsigned char BCE, unsigned char BCEcalculado, unsigned char recepcion){
...
void selector::seleccionarInterfaz(pcap_if *listaInterfaz, unsigned int max){
        ...
        if(interface < max){
            ...
            setDeviceName(aux, listaInterfaz[interface].name);      // aux = listaInterfaz[interface].name -> aux = nombre de la interfaz seleccionada.
            std::cout<<aux->deviceName<<std::endl;
            std::cout<<"La MAC es: ";

            GetMACAdapter(aux);                                     // aux = Adaptador de la MAC de la interfaz seleccionada.

            interfaz = *aux;                                        // Atributo interfaz = Adaptador de la MAC.
