...
void selector::recibirConfirmacion(){
        ...
        while(condition){
            ...
            if(data != nullptr){
                ...
                delete[] data;  //El paquete deja de ser util, se borra de memoria.
            }
        }       
            ...
            if(funcionamiento == 1){
                    if(car != 27){
                        ...
                        if(kbhit()){
                            car = getch();
                            if(car != 27){
                                frameB.enviarTrama((unsigned char*)car, 1, MAC_maestro);  // La primera MAC que cojo es la origen y a la que tengo que enviar los datos.
                            }
                        }   
                    }else{
                        funcionamiento = 0;
    ...
    if(emisorOreceptor){
        ...
        while(!ultimaTrama){
            ...
            while(!respondido){
                ...
                respondido = (*(aux+1) == ACK);
                //True si es ACK
                //False si no es ACK (NACK)
                       
                frameB.mostrarTramasControl((unsigned char*) aux, 0, 0, 'R');
