#include "FrameBuilder.h"
    ...
    std::string FrameBuilder::filtrarTrama(unsigned char* tramaRecibida, int longitud, int &nuevaLongitud, int filtroBajo, int filtroAlto){
       
        nuevaLongitud = longitud - (filtroBajo + filtroAlto);

        std::string datosFiltrados = "";

        
        
        for(int i = 0; i<nuevaLongitud; i++){
            datosFiltrados += tramaRecibida[i+filtroBajo];
        }
